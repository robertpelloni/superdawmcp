...
def main():
    parser = argparse.ArgumentParser(description="SuperDAW ProTools Pro Adapter CLI")
    ...
    try:
        if args.command == "transport":
            ...
            client.transport_control(playing, args.bpm, daw="protools")
            print(f"ProTools Pro Transport: {args.action} (BPM: {args.bpm})")

        elif args.command == "mixer":
            client.set_mixer(args.track_id, args.vol if args.vol is not None else 0.8, args.pan if args.pan is not None else 0.0, daw="protools")
            print(f"ProTools Pro Mixer: Track {args.track_id} (Vol: {args.vol}, Pan: {args.pan})")

        elif args.command == "create-track":
            client.create_track(args.name, args.type, daw="protools")
            print(f"ProTools Pro Track Created: {args.name} ({args.type})")
