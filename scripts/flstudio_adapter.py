...
def main():
    parser = argparse.ArgumentParser(description="SuperDAW FL Studio Adapter CLI")
    ...
    try:
        if args.command == "transport":
            ...
            client.transport_control(playing, daw="flstudio")
            print(f"FL Studio Transport: {args.action}")

        elif args.command == "mixer":
            client.set_mixer(args.track_id, args.vol if args.vol is not None else 0.8, args.pan if args.pan is not None else 0.0, daw="flstudio")
            print(f"FL Studio Mixer: Track {args.track_id} (Vol: {args.vol}, Pan: {args.pan})")

        elif args.command == "create-track":
            client.create_track(args.name, args.type, daw="flstudio")
            print(f"FL Studio Track Created: {args.name} ({args.type})")
